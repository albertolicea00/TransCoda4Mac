# Contributing to TransCoda for macOS
...
1. **Stay native.** Swift + SwiftUI only. No cross-platform UI layers, no web
...
[TransCoda4Windows](https://github.com/albertolicea00/TransCoda4Windows) is an
independent repository implementing the same architecture natively on WinUI 3.
...
2. `brew install xcodegen ffmpeg`
3. `xcodegen && open TransCoda.xcodeproj`

## Coding style

- Follow the [Swift API Design Guidelines](https://www.swift.org/documentation/api-design-guidelines/).
- SwiftUI views stay small; anything that talks to a process lives in `Core/`.
- FFmpeg arguments are always built as argument **lists**, never concatenated
  into shell strings.
...
docs: document hardware encoder detection
```
...
1. macOS version and app version.
